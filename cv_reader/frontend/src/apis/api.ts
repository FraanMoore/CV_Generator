...
export async function fetchCvMaster(): Promise<CVMasterRaw> {
  const res = await fetch(`${BASE_URL}/edit-cv`);
  if (!res.ok) {
    throw new Error('Error fetching CV master');
  }
  return res.json();
}